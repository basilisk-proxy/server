...
basilisk.proxy.use(path_rules.has_prefix_in({"/api/v2/users", "/api/v2/orders", "/api/v2/products"}), function(req, res, next)
  res:forward_headers("X-API-Version", "v2")
  return next()
end)

-- Apply authentication to all admin routes
basilisk.proxy.use(path_rules.has_prefix_in({"/admin", "/admin/settings", "/admin/reports"}), function(req, res, next)
...
basilisk.proxy.use(path_rules.has_prefix_in({"/api/public/search", "/api/public/browse"}), function(req, res, next)
